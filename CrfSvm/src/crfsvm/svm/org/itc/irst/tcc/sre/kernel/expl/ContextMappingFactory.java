...
public class ContextMappingFactory {
     ...
     * Logger instance named <code>ContextMappingFactory</code>.
     */
    static Logger logger = Logger.getLogger(ContextMappingFactory.class.getName());
    /**
    ...
    /**
     * Constructs a <code>ContextMappingFactory</code> object.
     *
     */
    private ContextMappingFactory() {
        ...
        try {
            ...
            if (configFile == null) {
                logger.debug("ContextMappingFactory uses the default config file: jsre-config.xml");
                mappingList = (List) digester.parse("jsre-config.xml");
            } else {
                logger.debug("ContextMappingFactory uses the config file: " + configFile);
     ...
     */
    public AbstractMapping getInstance(String name, String mode) throws MappingNotFoundException {
        ...
        try {
            ...
            for (int i = 0; i < mappingList.size(); i++) {
                ...
                if (mappingParameters.getName().equals(name)) {
                     ...
                     * Dung
                     */
                    mappingParameters.setParameters("mode", mode);
                    /*
                     * end Dung
                     */
                    ...
                    mapping = (AbstractMapping) (Class.forName(mappingParameters.getClassName())).newInstance();
     ...
     * <code>ContextMappingFactory</code>.
     *
    ...
    /**
     * Returns <code>ContextMappingFactory</code> object; only
     * one <code>ContextMappingFactory</code> instance can
     * exist.
     *
    ...
    public static void main(String args[]) throws Exception {
        ...
        if (args.length < 2) {
            System.err.println("java -mx512M org.itc.irst.tcc.sre.kernel.expl.ContextMappingFactory Mapping word+");
            System.exit(0);
        }

        ContextMappingFactory mappingFactory = ContextMappingFactory.getContextMappingFactory();
//		Mapping mapping = mappingFactory.getInstance(args[0]);
//		logger.info(mapping);	
    } // end main
} // end class ContextMappingFactory