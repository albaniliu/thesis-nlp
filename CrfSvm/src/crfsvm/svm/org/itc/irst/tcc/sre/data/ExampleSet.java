...
import java.util.*;
import crfsvm.svm.org.itc.irst.tcc.sre.util.FreqSet;
...
public abstract class ExampleSet implements ExampleSetReader, ExampleSetWriter {
     ...
     */
    static Logger logger = Logger.getLogger(ExampleSet.class.getName());
    //
    ...
    /*
     * Dung: Tao B bagging tu 1 set cho truoc, moi bagging gom k phan tu
     */
    ...
    /**
     * Ghep them 1 set vao set cho truoc
     * @param set
     * @return 
     */
    public ExampleSet merge(ExampleSet set) {
    ...
    /**
     * Loai bo tap con cua 1 set cho truoc
     * @param set
     * @return 
     */
    public ExampleSet removeSubSet(ExampleSet set) {
        for (int i = 0; i < set.size(); i++) {
            Object id = set.id(i);
            if (identifiers.contains(id)) {
                /*
                 * Tim ra vi tri cua phan tu i cua tap con set trong tap set cho truoc
                 */
                int index = identifiers.indexOf(id);
                /*
                 * Loai bo phan tu nay trong tap set cho truoc
                 */
                classes.remove(index);
                identifiers.remove(index);
                instances.remove(index);
            }// end if instances.contains(x) && classes.contains(y) && identifiers.contains(id)
        }// end for i
        return this;
    }// end subSet method
