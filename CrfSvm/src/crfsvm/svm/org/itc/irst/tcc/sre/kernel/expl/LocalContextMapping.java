...
import crfsvm.svm.org.itc.irst.tcc.sre.data.context.TargetContext;
import crfsvm.svm.org.itc.irst.tcc.sre.data.context.SecondTargetContext;
import crfsvm.svm.org.itc.irst.tcc.sre.data.context.AgentContext;
 ...
 * @since		1.0
 */
public class LocalContextMapping extends AbstractMapping implements ContextMapping {

    /**
     * Define a static logger variable so that it references the
     * Logger instance named <code>LocalContextMapping</code>.
     */
    static Logger logger = Logger.getLogger(LocalContextMapping.class.getName());
    //
    ...
    public Vector[] map(Object x, Object id, FeatureIndex[] index) throws IllegalArgumentException {
        ...
        for (int i = 0; i < sent.length(); i++) {
            // Lay ra vi tri cua agent va target trong cau
        ...
        switch (mode) {
            case 1:
                /*
                 * Set lai windowsize = A --> T
                 */
                if (agentOffset != -1 && targetOffset != -1 && targetOffset > agentOffset) {
                    windowSize = targetOffset - agentOffset;
                    targetContext.setSize(windowSize);
                    secondTargetContext.setSize(windowSize);
                    agentContext.setSize(windowSize);
                }// end if
    ...
    protected Vector createSubspace(Sentence sent, FeatureIndex index) {
        ...
        for (int i = 0; i < sent.length(); i++) {
            ...
            /*
            // ACE2005
            String attr = sent.wordAt(i).getLemma();
            String[] s = attr.split("_");
            if (s.length >= 3)
            {
            // head
            //addFeature(s[0] + "_HEAD", offset, vec, index);
...
///
//
    protected void createSubspace(Vector vec, Sentence sent, FeatureIndex index, String prefix) {
        //Vector vec = new SparseVector();
        //logger.debug("sent: " + sent.length());
        //
        for (int i = 0; i < sent.length(); i++) {
            ...
            // word form
            addFeature(prefix + "_" + form + Orthographic.WORD_FORM, offset, vec, index);

            // word stem
            addFeature(prefix + "_" + stem + Orthographic.STEM, offset, vec, index);

            // pos
            addFeature(prefix + "_" + pos + Orthographic.PART_OF_SPEECH, offset, vec, index);

            // uppercase
            if (Orthographic.isUpperCase(form)) //addFeature(form + Orthographic.form, offset, vec, index);
            {
                addFeature(prefix + "_" + Orthographic.UPPER_CASE, offset, vec, index);
            }

            // lowercase
            if (Orthographic.isLowerCase(form)) //addFeature(form + Orthographic.UPPER_CASE, offset, vec, index);
            {
                addFeature(prefix + "_" + Orthographic.LOWER_CASE, offset, vec, index);
            }

            // punctuation
            if (Orthographic.isPunctuation(form)) //addFeature(form + Orthographic.PUNCTUATION, offset, vec, index);
            {
                addFeature(prefix + "_" + Orthographic.PUNCTUATION, offset, vec, index);
            }

            // capitalized
            if (Orthographic.isCapitalized(form)) //addFeature(form + Orthographic.CAPITALIZED, vec, index);
            {
                addFeature(prefix + "_" + Orthographic.CAPITALIZED, offset, vec, index);
            }

            // numeric
            if (Orthographic.isNumeric(form)) //addFeature(form + Orthographic.NUMERIC, vec, index);
            {
                addFeature(prefix + "_" + Orthographic.NUMERIC, offset, vec, index);
            }
    ...
    //
    protected void createSubspace(Vector vec, Sentence st, Sentence sa, FeatureIndex index) {
        //Vector vec = new SparseVector();
        //logger.debug("sent: " + sent.length());
        //
        for (int i = 0; i < st.length(); i++) {
            ...
            for (int j = 0; j < sa.length(); j++) {			//String lemma = sent.wordAt(i).getLemma();
                String form = st.wordAt(i).getForm(false) + "_" + sa.wordAt(j).getForm(false);
                String stem = st.wordAt(i).getStem() + "_" + sa.wordAt(j).getStem();
                String pos = st.wordAt(i).getPos() + "_" + sa.wordAt(j).getPos();
