...
import crfsvm.svm.org.itc.irst.tcc.sre.data.ArgumentSet;
...
import crfsvm.svm.org.itc.irst.tcc.sre.kernel.expl.AbstractMapping;
import crfsvm.svm.org.itc.irst.tcc.sre.kernel.expl.ContextMappingFactory;
import crfsvm.svm.org.itc.irst.tcc.sre.util.FeatureIndex;
import crfsvm.svm.org.itc.irst.tcc.sre.util.Node;
...
public class CompareExampleSet
{
	 ...
	 * Logger instance named <code>CompareExampleSet</code>.
	 */
	static Logger logger = Logger.getLogger(CompareExampleSet.class.getName()); 
	...
	public void run() throws Exception
	{
		...
		AbstractMapping contextMapping = contextMappingFactory.getInstance(parameter.getProperty("kernel-type"), "-1");
	...
	private void compare()
	{
		...
		if (parameter.getProperty("kernel-threshold") != null)
			kernelThreshold = Double.parseDouble(parameter.getProperty("kernel-threshold"));
		...
		for (int i=0;i<outputSet1.size();i++)
		{
			for (int j=0;j<outputSet2.size();j++)
			{
				...
				//if (dot < kernelThreshold)
				{
					...
					//logger.info("v1: " + v1);
					//logger.info("v2: " + v2);
					//logger.info(total + "> " + i + ",  " + j + ": " + dot + " " + s1  + " <> " + s2 + " " + inputSet1.y(i) + " <> " + inputSet2.y(j) + "\n");
					//logger.info(i + ",  " + j + ": " + dot + " " + s2 + " <> " + inputSet2.y(j) + "\n");
	...
	public static void main(String args[]) throws Exception
	{
		...
		for (int i=0;i<args.length;i++)
		{
			...
			else if (args[i].equals("-t") || args[i].equals("--kernel-threshold"))
