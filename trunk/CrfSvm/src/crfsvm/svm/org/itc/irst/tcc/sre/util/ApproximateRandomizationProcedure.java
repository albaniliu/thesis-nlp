 ...
 */
package crfsvm.svm.org.itc.irst.tcc.sre.util;
...
{	
	 ...
	 * Logger instance named <code>ApproximateRandomizationProcedure</code>.
	 */
	 ...
	 */ 
	public ApproximateRandomizationProcedure(File goldFile, File baselineFile, File preferredFile, int n, double c) throws IOException, IndexOutOfBoundsException
	{
	...
	private double test(Evaluator[] baselineEval, Evaluator[] preferredEval)
	{
		...
		double sum = 0;
		double ssum = 0;
		logger.info("original score bs, ps,d: " + formatter.format(bs * 100) + "%, " + formatter.format(ps * 100) + "%, " + formatter.format(d * 100) + "%");
		...
		for (int i=0;i<iterations;i++)
		{
			...
			System.arraycopy(baselineEval, 0, baselineEvalCopy, 0, baselineEval.length);
			System.arraycopy(preferredEval, 0, preferredEvalCopy, 0, preferredEvalCopy.length);
	...
	public static void main(String args[]) throws Exception
	{
		...
		new ApproximateRandomizationProcedure(new File(g), new File(b), new File(p), n, c);
...
class Output
{
	/**
	 * Define a static logger variable so that it references the
	 * Logger instance named <code>Output</code>.
	 */
