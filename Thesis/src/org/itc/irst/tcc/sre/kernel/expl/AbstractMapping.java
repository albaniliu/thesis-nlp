...
public abstract class AbstractMapping implements Mapping {
     ...
     * Logger instance named <code>AbstractMapping</code>.
     */
    static Logger logger = Logger.getLogger(AbstractMapping.class.getName());
    //
    protected static final int SUBSPACE_DIMENSION = 25000;

    // TO DO
    public abstract Vector[] map(Object x, Object id, FeatureIndex[] index) throws IllegalArgumentException;
    ...
    protected Vector mergeSubspaces(Vector[] subspaces) {
        ...
        for (int j = 0; j < subspaces.length; j++) {
            ...
            while (it.hasNext()) {
                ...
                result.add((j * SUBSPACE_DIMENSION + e.getIndex()), e.getValue());
            }
        } // end for j
    ...
    {
    if (subspaces[i].size() > max)
    max = subspaces[i].size();
    } // end for i
